...
Tablespace::Tablespace(char obj_name, Database* database)
    : Object{obj_name}, database_{database}, pages_{} {}

Page::Page(char obj_name, Tablespace* tablespace)
    : Object{obj_name}, tablespace_{tablespace}, tuples_{} {}

Tuple::Tuple(char obj_name, Page* page) : Object{obj_name}, page_{page} {}

// Block from below --

void Database::block_from_below(Transaction* transaction,
                                LockType lock_request) {
  transaction->push_object_info(this);

  object_lock_info_[lock_request].insert(transaction->get_id());
}

void Tablespace::block_from_below(Transaction* transaction,
                                  LockType lock_request) {
  transaction->push_object_info(this);
...
void Tablespace::block_from_above(Transaction* transaction,
                                  LockType lock_request) {
  transaction->push_object_info(this);
